...
def crear_entradas():
    try:
        ...
        for i in range(n):
            fila = []
            for j in range(n):
                label = tk.Label(frame_matriz, text=f"a{chr(0x2081)}{chr(0x2081 + j)}:")
            ...
            label_resultado = tk.Label(frame_matriz, text=f"b{chr(0x2081 + i)}:")
...
label_n = tk.Label(root, text="Tamaño de la matriz (nxn) Debe ser cuadrada:", bg="#f0f0f0")
